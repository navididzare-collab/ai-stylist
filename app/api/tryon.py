...
SLEEVE_FIDELITY_RULE = (
    ...
    "BLAZER/SUIT JACKET vs VEST/WAISTCOAT — DO NOT CONFUSE THESE: These are two "
    "completely different garment types and you must not mistake one for the other. A "
    "BLAZER or SUIT JACKET always has full long sleeves reaching the wrist, and its own "
    "fabric covers the person's entire arm — if the reference photo shows lapels/collar "
    "AND long sleeves made of the same fabric as the body of the garment, it is a "
    "BLAZER/JACKET, not a vest, and you MUST render those same long sleeves on the "
    "person's arms in the output, in the jacket's own fabric and color; the arms must "
    "NOT be left bare or shown only in the inner shirt's sleeves. A VEST/WAISTCOAT/GILET "
    "has NO sleeves at all — it is only correctly a vest if the reference photo itself "
    "shows armholes with no fabric at all on the arms. Before generating the output, "
    "explicitly check: does the reference garment photo show fabric continuing down the "
    "full length of both arms to the wrist? If yes, this is a jacket/blazer with sleeves "
    "and the output must show full sleeves in that fabric. If no (the reference shows "
    "open armholes and no arm fabric), only then is it a vest and the person's arms "
    "should be covered solely by whatever inner layer they already have on. When in "
    "doubt, re-examine the reference photo for sleeve fabric before deciding — do not "
    "assume a tailored/structured outer garment is a vest just because it looks slim, "
    "cropped, or is worn open at the front."
)
    ...
    content = [
        {
            ...
            "text": (
                ...
                "(such as a jacket, coat, cardigan, overshirt, blazer/suit jacket — which has "
                "long sleeves — or, separately, a true sleeveless vest/waistcoat/gilet — which "
                "has no sleeves at all. Do not mix these two up; see the SLEEVE ACCURACY rule "
                "below for exactly how to tell them apart). Replace ONLY the "
